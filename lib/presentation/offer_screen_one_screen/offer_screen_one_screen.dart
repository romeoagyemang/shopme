import '../offer_screen_one_screen/widgets/offerscreenone_item_widget.dart';
...
class OfferScreenOneScreen extends StatelessWidget {
  const OfferScreenOneScreen({Key? key}) : super(key: key);
  ...
  /// Section Widget
  Widget _buildOfferScreenOne(BuildContext context) {
    return Expanded(
        child: BlocSelector<OfferScreenOneBloc, OfferScreenOneState,
                OfferScreenOneModel?>(
            selector: (state) => state.offerScreenOneModelObj,
            builder: (context, offerScreenOneModelObj) {
              return GridView.builder(
                  ...
                  itemCount:
                      offerScreenOneModelObj?.offerscreenoneItemList.length ??
                          0,
                  itemBuilder: (context, index) {
                    OfferscreenoneItemModel model =
                        offerScreenOneModelObj?.offerscreenoneItemList[index] ??
                            OfferscreenoneItemModel();
                    return OfferscreenoneItemWidget(model);
                  });
            }));
  }
